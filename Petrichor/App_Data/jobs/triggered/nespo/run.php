...
$business_id = 49; //nespo
$xml_link = "http://www.nespo.gr/client/products.xml";
$xml_link = $conn->real_escape_string($xml_link);
$xml_link = trim(stripslashes($xml_link));

//UPDATE modified date
$update_business_xml = $conn->query('UPDATE business_xml SET date_modified="' . $date_modified . '" WHERE business_id=49');		
...
while($reader->read()) {
    if($reader->nodeType == XMLReader::ELEMENT && $reader->name == 'product') {
        ...
        $manufacturer = mysqli_real_escape_string($conn,stripslashes(trim(mb_strtolower(trim($manufacturer)))));
        $instock = "Y";
...
$rows_new_products = mysqli_num_rows($insert_business_xml);
...
$stmt->execute();
$rows_missing_info = $stmt->num_rows;
...
$rows_update_products = mysqli_num_rows($update_business_xml);


$insert_messages = "Your XML file has been updated successfully! \n\n <strong>Reports</strong> \n New products: " . $rows_new_products . " \n update products: " . $rows_update_products . " \n Missing info: " . $rows_missing_info;
